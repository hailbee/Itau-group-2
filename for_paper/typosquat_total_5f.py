...
LIBRE_TEST = "../Libre/typosquat_test_pairs_with_siglip_embeddings.parquet"
LIBRE_PT = "../Libre/single_run_model.pt"

DOULOS_TEST = "../Doulos/typosquat_test_pairs_with_siglip_embeddings.parquet"
DOULOS_PT = "../Doulos/single_run_model.pt"

COUSINE_TEST = "../Cousine/typosquat_test_pairs_with_siglip_embeddings.parquet"
COUSINE_PT = "../Cousine/single_run_model.pt"

SAVED_MODEL_PATH = "saved_models/total_5f_model.joblib"
...
LIBRE_FRAUD_PREFIX = "fraud_emb_"
LIBRE_REAL_PREFIX = "real_emb_"
DOULOS_FRAUD_PREFIX = "fraud_emb_"
DOULOS_REAL_PREFIX = "real_emb_"
COUSINE_FRAUD_PREFIX = "fraud_emb_"
COUSINE_REAL_PREFIX = "real_emb_"
...
def choose_threshold_max_precision_under_recall_constraint(
    ...
    for thr in thresholds:
        m = evaluate_at_threshold(y_true, scores, float(thr))
    ...
    candidates.sort(
        key=lambda d: (d["precision"], d["recall"], d["accuracy"], d["threshold"]),
        reverse=True,
    )
    return candidates[0]
...
def main() -> None:
    ...
    df_te_li = load_table(LIBRE_TEST)
    df_te_do = load_table(DOULOS_TEST)
    df_te_co = load_table(COUSINE_TEST)

    for df in (df_te_txt, df_te_dj, df_te_uf, df_te_li, df_te_do, df_te_co):
        if LABEL_COL not in df.columns:
    ...
    proj_li, in_dim_li = load_golden_projector(LIBRE_PT, device=device)
    proj_do, in_dim_do = load_golden_projector(DOULOS_PT, device=device)
    proj_co, in_dim_co = load_golden_projector(COUSINE_PT, device=device)

    X_te_txt, _ = build_text_features(
        df=df_te_txt,
        fraud_col=FRAUD_COL,
        real_col=REAL_COL,
        label_col=LABEL_COL,
    ...
    cos_te_dj, _ = build_single_font_cosine(
        df=df_te_dj,
        label_col=LABEL_COL,
        positive_label=POSITIVE_LABEL,
    ...
    cos_te_uf, _ = build_single_font_cosine(
        df=df_te_uf,
        label_col=LABEL_COL,
        positive_label=POSITIVE_LABEL,
    ...
    cos_te_li, _ = build_single_font_cosine(
        df=df_te_li,
        label_col=LABEL_COL,
        positive_label=POSITIVE_LABEL,
        fraud_prefix=LIBRE_FRAUD_PREFIX,
        real_prefix=LIBRE_REAL_PREFIX,
        projector=proj_li,
        projector_in_dim=in_dim_li,
        device=device,
        batch_size=PT_BATCH_SIZE,
    )
    cos_te_do, _ = build_single_font_cosine(
        df=df_te_do,
        label_col=LABEL_COL,
        positive_label=POSITIVE_LABEL,
        fraud_prefix=DOULOS_FRAUD_PREFIX,
        real_prefix=DOULOS_REAL_PREFIX,
        projector=proj_do,
        projector_in_dim=in_dim_do,
        device=device,
        batch_size=PT_BATCH_SIZE,
    )
    cos_te_co, _ = build_single_font_cosine(
        df=df_te_co,
        label_col=LABEL_COL,
        positive_label=POSITIVE_LABEL,
        fraud_prefix=COUSINE_FRAUD_PREFIX,
        real_prefix=COUSINE_REAL_PREFIX,
        projector=proj_co,
    ...
    feature_names = list(saved_feature_names) if saved_feature_names is not None else default_feature_names
    ...
    for target in targets:
        result = choose_threshold_max_precision_under_recall_constraint(y_te, scores, min_recall=target)
        ...
        if abs(target - 0.975) < 1e-12:
            y_pred = (scores >= result["threshold"]).astype(np.int32)
    ...
    summary_path = os.path.join(ERROR_OUTPUT_DIR, "typosquat_total_5f_metrics.txt")
...
"""
python3 for_paper/typosquat_total_5f.py
"""