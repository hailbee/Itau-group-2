...
def main() -> None:
    ...
    # Downloads split (different naming scheme)
    ...
    ap.add_argument("--model-output-path", default="saved_models/small_model.joblib")
    ...
    # Basic alignment checks (so the 2-feature ensemble is meaningful)
    if len(df_tr_dl) != len(df_tr_dj):
        raise RuntimeError(f"Train row-count mismatch: Downloads={len(df_tr_dl)} Deja={len(df_tr_dj)}")
    if len(df_te_dl) != len(df_te_dj):
        raise RuntimeError(f"Test row-count mismatch: Downloads={len(df_te_dl)} Deja={len(df_te_dj)}")

    if args.label_col not in df_tr_dl.columns or args.label_col not in df_tr_dj.columns:
        raise RuntimeError(f"Missing label_col={args.label_col!r} in one of the TRAIN tables.")
    if args.label_col not in df_te_dl.columns or args.label_col not in df_te_dj.columns:
        raise RuntimeError(f"Missing label_col={args.label_col!r} in one of the TEST tables.")

    y_tr_dl = (df_tr_dl[args.label_col].to_numpy() == args.positive_label).astype(np.int32)
    y_tr_dj = (df_tr_dj[args.label_col].to_numpy() == args.positive_label).astype(np.int32)
    if not np.array_equal(y_tr_dl, y_tr_dj):
        raise RuntimeError("Train labels mismatch between Downloads and Deja tables (row order / dataset mismatch).")

    y_te_dl = (df_te_dl[args.label_col].to_numpy() == args.positive_label).astype(np.int32)
    y_te_dj = (df_te_dj[args.label_col].to_numpy() == args.positive_label).astype(np.int32)
    if not np.array_equal(y_te_dl, y_te_dj):
        raise RuntimeError("Test labels mismatch between Downloads and Deja tables (row order / dataset mismatch).")
    ...
    # Compute 1 cosine feature per dataset per split
    ...
    cos_tr_dj, y_tr2 = cosine_feature_from_dataset(
        df=df_tr_dj,
        label_col=args.label_col,
        positive_label=args.positive_label,
    ...
    cos_te_dj, y_te2 = cosine_feature_from_dataset(
        df=df_te_dj,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.deja_fraud_prefix,
    ...
    # Sanity: labels must match across features
    if not np.array_equal(y_tr, y_tr2):
        raise RuntimeError("Train labels mismatch after feature extraction (Downloads vs Deja).")
    if not np.array_equal(y_te, y_te2):
        raise RuntimeError("Test labels mismatch after feature extraction (Downloads vs Deja).")

    # Build 2-feature matrices
    X_tr = np.stack([cos_tr_dl, cos_tr_dj], axis=1).astype(np.float32, copy=False)
    X_te = np.stack([cos_te_dl, cos_te_dj], axis=1).astype(np.float32, copy=False)
    feat_names = ["cosine_downloads", "cosine_deja"]

    # -----------------------------
    # Ensemble (2 cosine features)
...
python3 for_paper/small.py \
  ...
  --model adaboost \
  --model-output-path saved_models/small_model.joblib
"""