...
def save_scatter_plot(summary_df: pd.DataFrame, out_png: str) -> None:
    ...
    plt.title("Image ensemble false-negative rate by spoof classification")
    plt.tight_layout()
...
def save_grouped_bar_chart(summary_df: pd.DataFrame, out_png: str) -> None:
    ...
    plt.ylabel("Percent")
    plt.title("Image ensemble spoof classification prevalence vs false-negative rate")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_png, dpi=200, bbox_inches="tight")
    plt.close()
...
def main() -> None:
    ...
    # Use the Deja test dataframe as the row-aligned base table for names/labels
    ...
    positives_parquet = os.path.join(args.error_output_dir, "image_ensemble_test_positives_multihot.parquet")
    fn_parquet = os.path.join(args.error_output_dir, "image_ensemble_false_negatives_multihot.parquet")
    summary_csv = os.path.join(args.error_output_dir, "image_ensemble_mechanism_error_summary.csv")
    summary_txt = os.path.join(args.error_output_dir, "image_ensemble_mechanism_error_summary.txt")
    scatter_png = os.path.join(args.error_output_dir, "image_ensemble_mechanism_error_scatter.png")
    bar_png = os.path.join(args.error_output_dir, "image_ensemble_mechanism_error_bar.png")
    ...
    length_summary_df = summarize_length_accuracy(length_df, max_bins=args.max_length_bins)

    length_samples_csv = os.path.join(args.error_output_dir, "image_ensemble_length_samples.csv")
    length_summary_csv = os.path.join(args.error_output_dir, "image_ensemble_length_summary.csv")
    length_curve_png = os.path.join(args.error_output_dir, "image_ensemble_length_accuracy_curve.png")
    length_bar_png = os.path.join(args.error_output_dir, "image_ensemble_length_accuracy_bar.png")

    length_df.to_csv(length_samples_csv, index=False)
    length_summary_df.to_csv(length_summary_csv, index=False)
    save_length_curve_plot(length_summary_df, length_curve_png)
    save_length_bar_plot(length_summary_df, length_bar_png)

    print()
    print(f"[OK] wrote length samples csv: {length_samples_csv}")
    print(f"[OK] wrote length summary csv: {length_summary_csv}")
    print(f"[OK] wrote length curve plot: {length_curve_png}")
    print(f"[OK] wrote length bar plot: {length_bar_png}")
...
"""
python3 for_paper/image_ensemble_error.py \
