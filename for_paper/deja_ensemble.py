...
# Feature builders
# =========================
def build_deja_ensemble_features(
    df: pd.DataFrame,
    fraud_col: str,
    real_col: str,
    label_col: str,
    ...
    X = np.stack([deja_cos, tsr, lev_dist_score, pr], axis=1).astype(np.float32, copy=False)
    return X, y
...
def save_scatter_plot(summary_df: pd.DataFrame, out_png: str) -> None:
    ...
    plt.title("Deja ensemble false-negative rate by spoof classification")
    plt.tight_layout()
...
def save_grouped_bar_chart(summary_df: pd.DataFrame, out_png: str) -> None:
    ...
    plt.ylabel("Percent")
    plt.title("Deja ensemble spoof classification prevalence vs false-negative rate")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_png, dpi=200, bbox_inches="tight")
    plt.close()
...
def main() -> None:
    ...
    X_tr, y_tr = build_deja_ensemble_features(
        df=df_tr,
    ...
    X_te, y_te = build_deja_ensemble_features(
        df=df_te,
        fraud_col=args.fraud_col,
        real_col=args.real_col,
        label_col=args.label_col,
    ...
    positives_parquet = os.path.join(args.error_output_dir, "deja_ensemble_test_positives_multihot.parquet")
    fn_parquet = os.path.join(args.error_output_dir, "deja_ensemble_false_negatives_multihot.parquet")
    summary_csv = os.path.join(args.error_output_dir, "deja_ensemble_mechanism_error_summary.csv")
    summary_txt = os.path.join(args.error_output_dir, "deja_ensemble_mechanism_error_summary.txt")
    scatter_png = os.path.join(args.error_output_dir, "deja_ensemble_mechanism_error_scatter.png")
    bar_png = os.path.join(args.error_output_dir, "deja_ensemble_mechanism_error_bar.png")
...
"""
python3 for_paper/deja_ensemble.py \
