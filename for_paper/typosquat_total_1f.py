...
SAVED_MODEL_PATH = "saved_models/total_1f_model.joblib"
...
def main() -> None:
    ...
    n_te = len(df_te_txt)
    if len(df_te_dj) != n_te:
        raise RuntimeError(
            "Test row-count mismatch: "
            f"Downloads={n_te} Deja={len(df_te_dj)}"
        )
    ...
    summary_path = os.path.join(ERROR_OUTPUT_DIR, "typosquat_total_1f_metrics.txt")
...
"""
python3 for_paper/typosquat_total_1f.py
"""