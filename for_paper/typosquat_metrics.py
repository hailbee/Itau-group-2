...
SAVED_MODEL_PATH = "saved_models/metrics_model.joblib"
...
def main() -> None:
    ...
    for c in (LABEL_COL, FRAUD_COL, REAL_COL):
        if c not in df_te_txt.columns:
            raise RuntimeError(f"Missing required column: {c}")
    ...
    X_te_metrics, _ = build_text_features(
        df=df_te_txt,
        fraud_col=FRAUD_COL,
        real_col=REAL_COL,
        label_col=LABEL_COL,
    ...
    available_features: Dict[str, np.ndarray] = {
        "token_set_ratio": X_te_metrics[:, 0],
        "levenshtein_distance_score": X_te_metrics[:, 1],
        "partial_ratio": X_te_metrics[:, 2],
    }
    ...
    if "text_cosine" in feature_names or any(name.startswith("cosine_") for name in feature_names):
        raise RuntimeError(
            f"metrics_model.joblib appears not to be a 3-text-metrics model. feature_names={feature_names}"
        )
    ...
    if hasattr(clf, "n_features_in_") and int(clf.n_features_in_) != X_te.shape[1]:
        raise RuntimeError(
            f"Saved model expects {int(clf.n_features_in_)} features but constructed matrix has {X_te.shape[1]}."
        )
    ...
    lines: List[str] = []
    lines.append("metrics_model_best:")
    lines.append("")
    ...
    for target in targets:
        ...
        if abs(target - 0.975) < 1e-12:
            ...
            fn_path, fp_path = save_error_lists(
                ...
                fn_filename="metrics_model_false_negatives_recall_ge_0.975.txt",
                fp_filename="metrics_model_false_positives_recall_ge_0.975.txt",
                fraud_col=FRAUD_COL,
                real_col=REAL_COL,
            )
    ...
    summary_path = os.path.join(ERROR_OUTPUT_DIR, "typosquat_metrics_model_results.txt")
...
"""
python3 for_paper/typosquat_metrics.py
"""