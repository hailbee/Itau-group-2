...
def main() -> None:
    ...
    ap.add_argument("--model-output-path", default="saved_models/deja_model.joblib")
    ...
    if args.label_col not in df_tr_dj.columns:
        raise RuntimeError(f"Missing label_col={args.label_col!r} in train table.")
    if args.label_col not in df_te_dj.columns:
    ...
    cos_tr_dj, y_tr = cosine_feature_from_dataset(
        df=df_tr_dj,
    ...
    cos_te_dj, y_te = cosine_feature_from_dataset(
        df=df_te_dj,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.deja_fraud_prefix,
    ...
    X_tr = cos_tr_dj.reshape(-1, 1).astype(np.float32, copy=False)
    X_te = cos_te_dj.reshape(-1, 1).astype(np.float32, copy=False)
...
python3 for_paper/deja.py \
  ...
  --model adaboost \
  --model-output-path saved_models/deja_model.joblib
"""