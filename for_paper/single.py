...
def build_text_metrics_features(
    df: pd.DataFrame,
    fraud_col: str,
    real_col: str,
    label_col: str,
...
def main() -> None:
    ...
    for name, df in train_tables:
        if len(df) != n_tr:
            raise RuntimeError(f"Train row-count mismatch: Downloads={n_tr} {name}={len(df)}")
    for name, df in test_tables:
        if len(df) != n_te:
            raise RuntimeError(f"Test row-count mismatch: Downloads={n_te} {name}={len(df)}")

    all_tables = [df_tr_dl, df_te_dl] + [df for _, df in train_tables] + [df for _, df in test_tables]
    ...
    cos_tr_dl, y_tr_dl2 = cosine_feature_from_dataset(
        df=df_tr_dl,
    ...
    cos_te_dl, y_te_dl2 = cosine_feature_from_dataset(
        df=df_te_dl,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.downloads_fraud_prefix,
    ...
    if not np.array_equal(y_tr, y_tr_dl2):
        raise RuntimeError("Train labels mismatch after feature extraction: Downloads")
    if not np.array_equal(y_te, y_te_dl2):
        raise RuntimeError("Test labels mismatch after feature extraction: Downloads")

    # Compute string metrics from Downloads text table
    X_tr_text_metrics, y_tr_tm = build_text_metrics_features(
        df=df_tr_dl,
    ...
    X_te_text_metrics, y_te_tm = build_text_metrics_features(
        df=df_te_dl,
        fraud_col=args.fraud_col,
        real_col=args.real_col,
        label_col=args.label_col,
    ...
    if not np.array_equal(y_tr, y_tr_tm):
        raise RuntimeError("Train labels mismatch after text metric extraction.")
    if not np.array_equal(y_te, y_te_tm):
        raise RuntimeError("Test labels mismatch after text metric extraction.")

    # Build master feature matrices in requested order
    ...
    for j, name in enumerate(feat_names):
        ...
        s_te = model_scores(clf, X_te_j).astype(np.float64)
        yhat_te = clf.predict(X_te_j)
