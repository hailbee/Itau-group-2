...
# Raw cosine
# =========================
def raw_cosine(fraud_mat: np.ndarray, real_mat: np.ndarray) -> np.ndarray:
...
# =========================
# PT loading + projection model (DEJA ONLY)
# =========================
...
def build_text_features_raw(
    ...
    text_cos = raw_cosine(fraud_mat, real_mat)
...
def build_font_feature_raw(
    ...
    cos = raw_cosine(fraud_mat, real_mat)
    return cos, y


def build_font_feature_projected(
    df: pd.DataFrame,
    label_col: str,
    positive_label: int,
    fraud_prefix: str,
...
def main() -> None:
    ...
    # Build text-side features (RAW cosine + 3 text features)
    X_tr_txt, y_tr_txt = build_text_features_raw(
        df=df_tr_txt,
    ...
    X_te_txt, y_te_txt = build_text_features_raw(
        df=df_te_txt,
        fraud_col=args.fraud_col,
        real_col=args.real_col,
        label_col=args.label_col,
    ...
    # Deja uses projected/model cosine
    proj_dj, in_dim_dj = load_golden_projector(args.deja_pt, device=device)
    cos_tr_dj, y_tr_dj = build_font_feature_projected(
        df=df_tr_dj,
        label_col=args.label_col,
        positive_label=args.positive_label,
    ...
    cos_te_dj, y_te_dj = build_font_feature_projected(
        df=df_te_dj,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.deja_fraud_prefix,
    ...
    # All other fonts use RAW cosine
    cos_tr_uf, y_tr_uf = build_font_feature_raw(
        df=df_tr_uf,
        label_col=args.label_col,
        positive_label=args.positive_label,
    ...
    cos_te_uf, y_te_uf = build_font_feature_raw(
        df=df_te_uf,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.unifont_fraud_prefix,
    ...
    cos_tr_li, y_tr_li = build_font_feature_raw(
        df=df_tr_li,
        label_col=args.label_col,
        positive_label=args.positive_label,
    ...
    cos_te_li, y_te_li = build_font_feature_raw(
        df=df_te_li,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.libre_fraud_prefix,
    ...
    cos_tr_ex, y_tr_ex = build_font_feature_raw(
        df=df_tr_ex,
        label_col=args.label_col,
        positive_label=args.positive_label,
    ...
    cos_te_ex, y_te_ex = build_font_feature_raw(
        df=df_te_ex,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.exo2_fraud_prefix,
    ...
    cos_tr_do, y_tr_do = build_font_feature_raw(
        df=df_tr_do,
        label_col=args.label_col,
        positive_label=args.positive_label,
    ...
    cos_te_do, y_te_do = build_font_feature_raw(
        df=df_te_do,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.doulos_fraud_prefix,
    ...
    cos_tr_co, y_tr_co = build_font_feature_raw(
        df=df_tr_co,
        label_col=args.label_col,
        positive_label=args.positive_label,
    ...
    cos_te_co, y_te_co = build_font_feature_raw(
        df=df_te_co,
        label_col=args.label_col,
        positive_label=args.positive_label,
        fraud_prefix=args.cousine_fraud_prefix,
    ...
    # text side: [raw_text_cosine, token_set_ratio, levenshtein_distance_score, partial_ratio]
    # image side: [deja_projected_cosine, unifont_raw_cosine, libre_raw_cosine,
    #              exo2_raw_cosine, doulos_raw_cosine, cousine_raw_cosine]
    ...
    positives_parquet = os.path.join(args.error_output_dir, "base_ensemble_test_positives_multihot.parquet")
    fn_parquet = os.path.join(args.error_output_dir, "base_ensemble_false_negatives_multihot.parquet")
    summary_csv = os.path.join(args.error_output_dir, "base_ensemble_mechanism_error_summary.csv")
    summary_txt = os.path.join(args.error_output_dir, "base_ensemble_mechanism_error_summary.txt")
    scatter_png = os.path.join(args.error_output_dir, "base_ensemble_mechanism_error_scatter.png")
    bar_png = os.path.join(args.error_output_dir, "base_ensemble_mechanism_error_bar.png")
...
"""
python3 for_paper/base_ensemble.py \
